...
#Read 2017 Kuparuk discharge data#
Q.data.2017 = as.data.frame(read_excel("Kuparuk_Q_2017.xlsx"))
#Convert timestamp from text to time R recognize#
Q.data.2017$Date = as.POSIXct(Q.data.2017$Date, format="%Y-%m-%d %H:%M:%S")
#Convert timestamp to minutes since start of the year#
Q.data.2017$min.since = as.numeric(difftime(Q.data.2017$Date, start.time, units="mins"))

#Interpolation function for Q~time#
Q.fun.2017 = approxfun(x=Q.data.2017$min.since, y=Q.data.2017$Discharge_m3s)
...
for(m in 1:length(window.size)){
	#number of windows#
	no.window = floor(with(chem.2017, (max(min.since)-min(min.since))/60/24)-window.size[m]+1)
	...
	for(i in 1:no.window){
		...
		data.temp = with(chem.2017, chem.2017[min.since>min(min.since)+(i-1)*24*60 & min.since<min(min.since)+(i+29)*24*60,])
	...
	julian.day = seq(from=floor(min(chem.2017$min.since)/60/24), by=1, length.out=no.window)
	
	#Plot the slope of the C-Q regression#
	title = paste0("2017, ",as.character(window.size[m])," days")
