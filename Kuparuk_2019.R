...
#Read 2019 Kuparuk discharge data#
Q.data.2019 = as.data.frame(read_excel("Kuparuk_Q_2019.xlsx"))
#Convert timestamp from text to time R recognize#
Q.data.2019$Date = as.POSIXct(Q.data.2019$Date, format="%Y-%m-%d %H:%M:%S")
#Convert timestamp to minutes since start of the year#
Q.data.2019$min.since = as.numeric(difftime(Q.data.2019$Date, start.time, units="mins"))

#Interpolation function for Q~time#
Q.fun.2019 = approxfun(x=Q.data.2019$min.since, y=Q.data.2019$Discharge_m3s)
...
for(m in 1:length(window.size)){
	#number of windows#
	no.window = floor(with(chem.2019, (max(min.since)-min(min.since))/60/24)-window.size[m]+1)
	...
	for(i in 1:no.window){
		...
		data.temp = with(chem.2019, chem.2019[min.since>min(min.since)+(i-1)*24*60 & min.since<min(min.since)+(i+29)*24*60,])
	...
	julian.day = seq(from=floor(min(chem.2019$min.since)/60/24), by=1, length.out=no.window)
	
	#Plot the slope of the C-Q regression#
	title = paste0("2019, ",as.character(window.size[m])," days")
